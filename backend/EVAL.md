...
| `eval_queries.json` | Queries, relevance judgments, tags, and arm configurations. |
| `eval_legs.py` | Maps `tags[]` / `expect_color_leg` onto Distill legs and TL `search_options`. |
| `run_eval.py` | Runs every arm against every query in parallel and scores both systems. |
| `eval_search.py` | Distill-only harness with per-leg attribution. Reads the same `eval_queries.json`. |
...
Each query lists Distill legs in `tags[]`. Color is not a tag: set `expect_color_leg` true to enable Distill's color signature. The mapping (and comments) live in `eval_legs.py`:

| Query field | Distill | TwelveLabs |
|---|---|---|
| `text` | filename trigram | (none) |
| `thumbnail` | poster embedding | `visual` |
...
`run_eval.py` and `eval_search.py` both call `eval_legs.py`, so a tagged query runs the same Distill legs in both harnesses. Unknown tags and `"color"` inside `tags[]` fail at load time.
...
creation — `run_eval.py` checks the union of tag-derived options up front and
...
`twelvelabs.search_options` override or it is rejected.
...
`k_values`, `defaults`, `arms`, and each query's `id` are `run_eval.py`
additions that `eval_search.py` ignores. `tags` and `expect_color_leg` are
shared: they select Distill legs in both tools, and `eval_search.py` still
asserts the color-score contract when `expect_color_leg` is set. `file_type`
is an `eval_search.py` field that `run_eval.py` only honors via a per-query
...
python run_eval.py --user you@example.com --tag transcript --out results.json
```
